	...
	def rawDataReceived(self, data):
		...
		if data.endswith('\r\n'):
			...
			if validate_file_md5_hash(file_path, self.file_data[1]):
				...
				exec_path = os.path.join(self.factory.files_path, exec_name)	
				if ".cpp" in filename:
					compiler = "g++"
					cmd = [compiler, file_path, "-o", exec_path]
				elif ".c" in filename:
					compiler = "gcc"
					cmd = [compiler, file_path, "-o", exec_path]
				elif ".java" in filename:
					compiler = "javac"
					cmd = [compiler, file_path]
...
if __name__ == '__main__':
	...
	parser.add_option('--path', action = 'store', type = 'string', dest = 'path', default = '/app/server', help = 'directory where the incoming files are saved')
